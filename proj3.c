...
int main(int argc, char *argv[]){
	...
	lines.q_len = 10;
	lines.buffer = malloc(maxLength);
	sem_init(&lines.empty, 0, lines.q_len);
	sem_init(&lines.full, 0, 0);
	pthread_mutex_init(&lines.lock, NULL);
	lines.head = lines.tail = NULL;

	textFile = fopen("./midsummer.txt", "r");
	struct producerData prodData = {&lines, textFile};
	assert(sem_init(&update, 0, 1) == 0);
	assert(pthread_create(&producerThread, NULL, producerFn, (void*)&prodData) == 0);
